...
const Github = () => {
  ...
  useEffect(() => {
    fetch("https://github-contributions-api.jogruber.de/v4/shrivarshapoojari")
      .then((res) => res.json())
      .then((json) => {
        const formattedData = json.contributions.map((item) => ({
          date: item.date,
          count: item.count,
        }));
  ...
  const handleMouseOver = (event, value) => {
    ...
    const content = `${date}: ${count} ${count === 1 ? "contribution" : "contributions"}`;
...
<div style={{ display: "flex", justifyContent: "space-between", marginTop: "8px" }}>
          <div style={{ fontSize: "20px" , marginTop: "10px"}}>{totalContributions} contributions last year</div>
