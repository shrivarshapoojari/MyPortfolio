...
import leetcodeFallbackData from "./leetcodeFallbackData";
...
const LeetCode = () => {
  ...
  useEffect(() => {
    const CACHE_KEY = "leetcode_calendar_cache";
    ...
    if (cached) 
      ...
      if (now - timestamp < CACHE_DURATION) {
        setData(cachedData);
        const total = cachedData.reduce((sum, item) => sum + item.count, 0);
    ...
    fetch("https://alfa-leetcode-api.onrender.com/shrivarsha/calendar/")
      ...
      .then((json) => {
        ...
        try {
          ...
          formattedData = Object.keys(parsedData).map((timestamp) => ({
            date: moment.unix(timestamp).format("YYYY-MM-DD"),
            count: parsedData[timestamp],
          }));
        } catch (e) {
          // fallback to imported data if API response is invalid
          formattedData = leetcodeFallbackData;
        }
      ...
      .catch((error) => {
        ...
        const formattedData = leetcodeFallbackData;
